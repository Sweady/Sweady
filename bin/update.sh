...
if [ "$1" == "scaleway" ] || [ "$1" == "aws" ] || [ "$1" == "openstack" ]; then
    ...
    if [ "$1" == "scaleway" ]; then
        ansible-playbook ./../../ansible/add_worker_swarm.ansible.yaml -i ../../tmp/inventory_ansible_swarm --user=root
    fi
    if [ "$1" == "aws" ]; then
        ansible-playbook ./../../ansible/add_worker_swarm.ansible.yaml -i ../../tmp/inventory_ansible_swarm --user=ubuntu
    fi
    if [ "$1" == "openstack" ]; then
        ansible-playbook ./../../ansible/add_worker_swarm.ansible.yaml -i ../../tmp/inventory_ansible_swarm --user=cloud
    fi
fi